...
BigInt powm(const BigInt& base, long long exp, BigInt m) {
	if (exp < 0) {
		if (base == 0)
			throw std::logic_error("Cannot divide by zero");
		return abs(base) == 1 ? base : 0;
	...
	while (exp > 1) {
		if (exp % 2)
			result_odd = (result_odd * result) % m;
		result = (result * result) % m;
		exp /= 2;
	}

	return (result * result_odd) % m;
}
...
/*
    BigInt + BigInt
    ---------------
    The oper&& on the RHS of the addition is `num`.
*/

BigInt BigInt::operator+(const BigInt& num) const {
    // if the oper&&s are of opposite signs, perform subtraction
    ...
    // if the oper&&s are negative, the result is negative
...
/*
    BigInt - BigInt
    ---------------
    The oper&& on the RHS of the subtraction is `num`.
*/

BigInt BigInt::operator-(const BigInt& num) const {
    // if the oper&&s are of opposite signs, perform addition
    if (this->sign == '+' && num.sign == '-') {
        BigInt rhs = num;
        rhs.sign = '+';
        return *this + rhs;
...
/*
    ...
    The oper&& on the RHS of the product is `num`.
*/
...
/*
    ...
    The oper&& on the RHS of the division (the divisor) is `num`.
*/
...
/*
    ...
    The oper&& on the RHS of the modulo (the divisor) is `num`.
*/
